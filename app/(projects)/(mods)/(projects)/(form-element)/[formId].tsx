...
//  * Preview + Save helper
//  */
// export const previewAndSaveForm = (
//   realm: any,
//   formData: Record<string, any>,
//   fields: FormField[] = []
// ) => {
//   try {
//     const previewSubmission = createSurveySubmission(formData, fields, realm);

//     const previewData = {
//       id: previewSubmission.id,
//       answers: previewSubmission.answers,
//       form_data: previewSubmission.form_data,
//       location: previewSubmission.location,
//       sync_data: {},
//     };

//     Alert.alert("Preview Form", JSON.stringify(previewData, null, 2), [
...
//           try {
//             realm.write(() => {
//               realm.create(SurveySubmission, {
//                 ...previewSubmission,
//                 _id: new BSON.ObjectId(),
//               });
//             });
//             Alert.alert("Saved", "Form submission saved locally.");
//           } catch (err) {
//             console.error("Error saving locally:", err);
//             Alert.alert("Error", "Could not save submission locally.");
//           }
//         },
//       },
//     ]);
//   } catch (err) {
//     console.error("previewAndSaveForm error:", err);
...
//   const pid = formId ? parseInt(formId, 10) : NaN;
//   const pmid = project_module_id ? parseInt(project_module_id, 10) : NaN;
//   const smid = source_module_id ? parseInt(source_module_id, 10) : NaN;
//   const projId = project_id ? parseInt(project_id, 10) : NaN;

//   // Hooks — always top-level
//   const { form: regularForm, isLoading } = useGetFormById(pid, pmid, smid, projId);
...
//   const localFormioJs = require("../../../../../assets/formio/formio.full.min.js");
//   const localFormioCss = require("~/assets/formio/formio.full.min.css");
//   const localBootstrapCss = require("~/assets/formio/bootstrap.min.css");

//   // Load assets once
//   useEffect(() => {
//     const loadAssets = async () => {
//       try {
//         await Asset.loadAsync([localFormioJs, localFormioCss, localBootstrapCss]);

//         const jsUri = Asset.fromModule(localFormioJs).localUri ?? Asset.fromModule(localFormioJs).uri;
//         const cssUri = Asset.fromModule(localFormioCss).localUri ?? Asset.fromModule(localFormioCss).uri;
//         const bootstrapUri = Asset.fromModule(localBootstrapCss).localUri ?? Asset.fromModule(localBootstrapCss).uri;

//         setLocalAssets({
//           js: jsUri || "https://cdn.form.io/formiojs/formio.full.min.js",
//           css: cssUri || "https://cdn.form.io/formiojs/formio.full.min.css",
//           bootstrap: bootstrapUri || "https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css",
//         });
//       } catch (error) {
//         console.warn("Local asset load failed, using CDN fallback:", error);
//         setLocalAssets({
...
//   // Parse form JSON safely (after hooks)
...
//             try {
//               await loadJS('${localAssets.js}');
//               const form = await Formio.createForm(
//                 document.getElementById('formio'),
//                 ${JSON.stringify(parsedForm)},
//                 { noAlerts: true, language: '${t("language", "en")}' }
//               );
...
//                   formName: '${regularForm.name || "Form"}',
...
//   const fields = parsedForm?.components || [];

//   const handleFormSubmission = (formData: any) => {
//     try {
//       previewAndSaveForm(realm, formData, fields);
//     } catch (error) {
...
// Fixed ProjectFormElementScreen.tsx - Resolved WebView and asset loading issues
...
//       time_spent_filling_the_form: Date.now(),
...
//   // Memoize parsed parameters to prevent unnecessary re-renders
...
//     const loadAssets = async () => {
//       try {
//         // Use CDN assets to avoid local asset loading issues
//         if (isMounted) {
//           setLocalAssets({
//             js: "https://cdn.form.io/formiojs/formio.full.min.js",
//             css: "https://cdn.form.io/formiojs/formio.full.min.css",
...
//       await saveFormSubmissionLocally(realm, formData, regularForm, user, fields);

//       Toast.show({
//         type: "success",
//         text1: t("Alerts.success.title") || "Success",
//         text2: t("Alerts.success.form_saved_locally") || "Form saved locally",
//         position: "top",
//         visibilityTime: 3000,
//       });
...
//     }
//   }, [realm, regularForm, user, fields, t, isSubmitting]);
...
//               loadingEl.innerHTML = '<div style="color: red;">Error: ' + message + '</div>';
...
//     </html>
//     `;
//   }, [localAssets, parsedForm, t, regularForm?.name]);
