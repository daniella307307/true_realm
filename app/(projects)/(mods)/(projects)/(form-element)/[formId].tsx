...
function ProjectFormElementScreen(): React.JSX.Element {
  ...
  // Replace your entire formHtml useMemo with this corrected version:
    ...
    (function() {
      ...
      function renderComponentValue(value, component) {
        ...
        function renderFileList(files) {
          let html = '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 10px; margin-top: 8px;">';
          files.forEach(function(file) {
            ...
            if (isImage) {
              html += '<div style="border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; background: #f9f9f9;">';
              html += '<img src="' + url + '" alt="' + name + '" style="width: 100%; height: 120px; object-fit: cover;" />';
              html += '<div style="padding: 8px; font-size: 12px; color: #666; text-align: center;">' + name + '</div>';
              html += '</div>';
            } else {
              html += '<div style="border: 1px solid #e0e0e0; border-radius: 8px; padding: 12px; background: #f9f9f9; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100px;">';
              html += '<i class="fas fa-file" style="font-size: 32px; color: var(--primary-color); margin-bottom: 8px;"></i>';
              html += '<div style="font-size: 12px; color: #666; text-align: center;">' + name + '</div>';
              html += '</div>';
            }
          });
          html += '</div>';
        ...
        if (Array.isArray(value)) {
          ...
          if (typeof value[0] === "object") {
            ...
            value.forEach(function(row, rowIndex) {
              ...
              html += '<div style="font-weight: 600; color: var(--primary-color); margin-bottom: 12px;">';
              html += '<i class="fas fa-list-ol" style="margin-right: 8px;"></i>';
              html += (component.label || component.key) + ' - ' + (rowIndex + 1);
              html += '</div>';
              html += '<div style="display: grid; gap: 10px;">';
              
              const subComponents = component.components || component.columns || component.rows || [];
              ...
              subComponents.forEach(function(sub) {
                ...
                html += '<div style="padding: 8px; background: white;border-left: 3px solid gray;">';
                html += '<div style="font-size: 13px; font-weight: 600; color: #495057; margin-bottom: 4px;">' + subLabel + '</div>';
                html += '<div style="font-size: 14px; color: #212529;">' + renderComponentValue(subValue, sub) + '</div>';
                html += '</div>';
              });
          ...
          let badgeHtml = '<div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 6px;">';
          value.forEach(function(v) {
            badgeHtml += '<span style="display: inline-flex; align-items: center; background: linear-gradient(135deg, var(--primary-color), var(--primary-light)); color: white; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 500;">';
            badgeHtml += '<i class="fas fa-check-circle" style="margin-right: 6px; font-size: 12px;"></i>';
            badgeHtml += String(v);
            badgeHtml += '</span>';
          });
          badgeHtml += '</div>';
        ...
        if (typeof value === "object") {
          ...
          let html = '<div style="margin-top: 10px; padding: 12px; background: #f8f9fa; border-radius: 6px;">';
          
          (component.components || []).forEach(function(sub) {
            const subKey = sub.key;
            const subLabel = sub.label || subKey;
            ...
            html += '<div style="font-size: 13px; font-weight: 600; color: #495057; margin-bottom: 4px;">' + subLabel + '</div>';
            html += '<div style="font-size: 14px;">' + renderComponentValue(subValue, sub) + '</div>';
            html += '</div>';
          });
        ...
        if (typeof value === "boolean") {
          if (value) {
            return '<span style="display: inline-flex; align-items: center; color: #28a745; font-weight: 500;"><i class="fas fa-check-circle" style="margin-right: 6px;"></i>' + reviewTranslations.yes + '</span>';
          } else {
            return '<span style="display: inline-flex; align-items: center; color: #dc3545; font-weight: 500;"><i class="fas fa-times-circle" style="margin-right: 6px;"></i>' + reviewTranslations.no + '</span>';
          }
        }
        
        if (component.type === 'select' || component.type === 'radio' || component.type === 'selectboxes') {
          return '<span style="display: inline-flex; align-items: center; background: linear-gradient(135deg, var(--primary-color), var(--primary-light)); color: white; padding: 8px 16px; border-radius: 20px; font-size: 14px; font-weight: 500;"><i class="fas fa-dot-circle" style="margin-right: 8px; font-size: 12px;"></i>' + String(value) + '</span>';
        }
        
        const textValue = String(value);
        if (textValue.length > 100) {
          return '<div style="padding: 12px; background: #f8f9fa; border-radius: 6px; line-height: 1.6; white-space: pre-wrap;">' + textValue + '</div>';
        }
        
        return '<span style="color: #212529; font-weight: 500;">' + textValue + '</span>';
      }
      ...
      async function initializeForm() {
        ...
        if (typeof Formio === 'undefined' || !window.Formio || !window.Formio.createForm) {
          ...
          if (initAttempts < MAX_INIT_ATTEMPTS) {
            ...
            const errorMsg = 'FormIO library failed to load.';
        ...
        try {
          ...
          if (formSchema.display === 'wizard' && Array.isArray(formSchema.components)) {
            formSchema.components.forEach(function(page) {
              if (page.components) {
