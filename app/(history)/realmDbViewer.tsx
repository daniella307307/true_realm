...
const RealmDatabaseViewer = () => {
  ...
  // Load pagination status on mount
  useEffect(() => {
  ...
  // Calculate paginated submissions for the selected project
  ...
  // Check if there are more submissions to load for the project
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // Refresh pagination status after sync
      const status = await getPaginationStatus(getAll);
      setPaginationMetadata(status.metadata);
      setCachedPages(status.cachedPages);
      setTotalCached(status.totalCached);
  ...
  const handleLoadMoreProjectSubs = () => {
    if (!projectHasNextPage || loadingMore) {
      return;
    }
  ...
  // Switch between the three main views
  switch (currentView) {
    case 'projects':
