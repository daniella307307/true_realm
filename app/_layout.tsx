...
//   const { getAll, update, create, query} = useSQLite();
...
//                 <AppDataProvider>
//                   <SyncProvider getAll={getAll} update={update} create={create} query={query} t={t} userId={userId} syncIntervalMs={30000} enabled={!!userId}>
//                      <Layout />
//                   </SyncProvider>
//                 </AppDataProvider>
//               </RouteProtectionProvider>
...
// function SyncStatus(){
//   const { isSyncing, lastSyncTime, pendingChanges, triggerManualSync} = useSync();
//   return(
// <View>
...
//       )}
//       {lastSyncTime && (
//         <Text>Last synced: {lastSyncTime.toLocaleTimeString()}</Text>
//       )}
...
// New component that uses SQLite hooks - now properly inside SQLiteProvider
function AppWithSQLite() {
  const { getAll, update, create, query } = useSQLite();
  const { t } = useTranslation();
  const { user: authUser } = useAuth({});
  const [user, setUser] = useState<LocalUser | null>(null);
